...
- **サーバー起動用のターミナル (閉じない)**: サーバーを起動したら、そのまま表示しておきます (ログが流れ続けます)。
  - 例: `./deployment/start_server_dev.sh` または `./deployment/start_server.sh`
  - 停止はこのターミナルで `Ctrl + C` (または、後述の停止コマンドを別ターミナルで実行)。
- **別のターミナル (必要時のみ開く)**: サーバーの状態確認や停止などを実行します:
  - 例: `./deployment/server_status.sh`, `./deployment/stop_server.sh`
...
注意:
- 開発モードと通常モードのサーバーを**同時に起動しない**でください (ポート競合)。
- 既に8000番ポートが使われている場合は、どちらかを停止してから起動するか、`--port` を変更してください。
